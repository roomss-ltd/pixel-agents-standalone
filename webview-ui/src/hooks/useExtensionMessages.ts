...
export function useExtensionMessages(
  ...
  const saveAgentSeats = (os: OfficeState): void => {
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        console.log('[Frontend] Adding', pendingAgents.length, 'pending agents to office');
        for (const p of pendingAgents) {
          console.log('[Frontend] Adding agent', p.id, 'with folder', p.folderName);
        ...
        os.removeAgent(id);
      } else if (msg.type === 'sessionInactive') {
        const id = msg.id as number;
        const agent = os.characters.get(id);
        if (agent) {
        ...
        console.log('[Frontend] Received existingAgents:', incoming);
  ...
  }, [getOfficeState, sendMessage, onLayoutLoaded, isEditDirty]);
