...
function App() {
  ...
  const handleWebSocketMessage = useCallback((message: Record<string, unknown>) => {
    // Forward WebSocket messages to window.postMessage so useExtensionMessages can handle them
  ...
  const {
    ...
    isEditDirty,
    isBrowserRuntime ? undefined : sendMessage,
  );
  ...
  const handleSelectAgent = useCallback((id: number) => {
    // In standalone mode, clicking an agent shows session info instead of focusing terminal
    if (!isBrowserRuntime) {
      setSelectedAgentForInfo(id);
    } else {
