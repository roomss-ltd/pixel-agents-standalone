...
export function renderScene(
  ...
  for (const ch of characters) {
    ...
    if (isSelected || isHovered) {
      let outlineAlpha = isSelected ? SELECTED_OUTLINE_ALPHA : HOVERED_OUTLINE_ALPHA;
      // Reduce outline opacity for inactive agents
      if (ch.isInactive) {
        outlineAlpha *= 0.4;
      }
    ...
    drawables.push({
      ...
      draw: (c) => {
        if (ch.isInactive) {
          c.save();
          c.globalAlpha = 0.4; // Gray out inactive agents
