...
test("peek view mode controls live in a bottom action row with window icons", () => {
  ...
  assert.match(styles, /\.peek-actions\s*\{[\s\S]*justify-content: flex-end;/);
...
test("idle peek renders useful all-clear context instead of an empty card", () => {
  ...
  assert.match(html, /if \(item && item\.kind === "idle"\) \{[\s\S]*var idleIcon = document\.getElementById\("coffee-idle-icon-template"\);[\s\S]*kind\.innerHTML = idleIcon \? idleIcon\.innerHTML : "";/);
