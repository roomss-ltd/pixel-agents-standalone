...
test("mini idle loader slot renders a muted coffee icon instead of the active blue dot", () => {
  ...
  assert.match(icons, /coffee-smoke coffee-smoke-1" d="M7\.1 7\.1C4\.2 5\.2 10\.1 5\.6 7\.4 4\.6/);
  assert.match(icons, /coffee-smoke coffee-smoke-2" d="M11 7\.1C8\.1 5\.1 14 3\.9 11\.2 1\.3/);
  assert.match(icons, /coffee-smoke coffee-smoke-3" d="M14\.9 7\.1C12 5\.2 17\.8 5\.6 15 4\.6/);
  ...
  assert.doesNotMatch(styles, /\.widget\.compact \.loader-slot:not\(\.is-active\) \.coffee-smoke-[23]\s*\{[\s\S]*?animation-delay:/);
  assert.match(styles, /@keyframes coffeeSteam/);
  assert.match(styles, /18%\s*\{[\s\S]*opacity: 0\.24;[\s\S]*stroke-dashoffset: 6\.2;/);
  assert.match(styles, /42%\s*\{[\s\S]*opacity: 0\.76;[\s\S]*stroke-dashoffset: 2\.8;/);
  assert.match(styles, /78%\s*\{[\s\S]*opacity: 0\.08;[\s\S]*stroke-dashoffset: 0;/);
...
test("idle peek renders useful all-clear context instead of an empty card", () => {
  ...
  assert.match(styles, /\.peek-ticker\.idle \.coffee-smoke\s*\{[^}]*animation: coffeeSteam 4\.8s ease-in-out infinite;/);
  ...
  assert.doesNotMatch(styles, /\.peek-ticker\.idle \.coffee-smoke-[23]\s*\{[\s\S]*?animation-delay:/);
