...
- 2026-05-03: Waiting event popovers changed from sticky duplicates to expiring attention pings. Rationale: a waiting row is already promoted into the primary Peek surface, so the separate notification should remind briefly instead of remaining permanent. `enqueueStatusEvent()` now gives waiting and finished events the same `EVENT_TTL`, `cleanupExpiredEvents()` no longer preserves waiting rows by active status, and the event renderer uses the local Lucide-style `triangle-alert` SVG instead of a bare `!` glyph. RED confirmed with `phase_h_event_notifications.test.mjs`; focused GREEN confirmed with `phase_h_event_notifications.test.mjs` passing 13/13. Full verification confirmed `node --test claude-tab-status/hammerspoon/*.test.mjs claude-tab-status/src/*.test.mjs` passing 183/183, `cargo build` passing, and `cargo test --no-run` passing.
- 2026-05-03: Peek waiting marker aligned with the notification icon. Runtime screenshot showed the primary Peek waiting card still rendered a bare `!` even though event notifications had moved to the Lucide-style warning icon. Root cause: Peek has an independent marker path through `peekKindLabel("waiting")`. The Peek renderer now uses the shared `triangle-alert-icon-template` for waiting items, and CSS sizes the inline SVG in the existing waiting marker pill. RED/GREEN confirmed with `phase_i_peek_mode.test.mjs`; full verification confirmed `node --test claude-tab-status/hammerspoon/*.test.mjs claude-tab-status/src/*.test.mjs` passing 183/183, `cargo build` passing, and `cargo test --no-run` passing.
- 2026-05-03: Peek hover history height increased after runtime screenshot showed `Recently Active` / `Older Finished` rows getting cramped behind the bottom action bar. The history list remains bounded and scrollable, but the active-stack history cap increased from `150px` to `220px`, and the native Peek hover frame cap increased from `420px` to `520px` so Lua frame sizing and CSS max-height stay aligned. RED/GREEN confirmed with `phase_i_peek_mode.test.mjs`; full verification confirmed `node --test claude-tab-status/hammerspoon/*.test.mjs claude-tab-status/src/*.test.mjs` passing 183/183, `cargo build` passing, and `cargo test --no-run` passing.
- 2026-05-03: Active blue Peek badges now match the bordered treatment used by completed green badges. The primary Peek tab badge and active-stack tab badges get an inset blue stroke (`rgba(115, 166, 255, 0.22)`) so processing tab numbers have the same contained chip shape as recent/older finished rows. RED/GREEN confirmed with `phase_i_peek_mode.test.mjs` and `phase_k_peek_active_queue.test.mjs`; full verification confirmed `node --test claude-tab-status/hammerspoon/*.test.mjs claude-tab-status/src/*.test.mjs` passing 183/183, `cargo build` passing, and `cargo test --no-run` passing.
- 2026-05-03: Waiting notification persistence narrowed to Mini mode only. Waiting events still get the normal `EVENT_TTL`, but `cleanupExpiredEvents()` now preserves an expired waiting event only while the current view is Mini/compact and the underlying session is still waiting. In Peek mode the promoted waiting card remains the persistent source of truth, so the separate event popover expires normally. RED/GREEN confirmed with `phase_h_event_notifications.test.mjs`; full verification confirmed `node --test claude-tab-status/hammerspoon/*.test.mjs claude-tab-status/src/*.test.mjs` passing 183/183, `cargo build` passing, and `cargo test --no-run` passing.
- 2026-05-03: Mini enlarge into idle Peek fixed. Root cause: `compactViewMode()` forced `activeTotal <= 0` back to Mini before honoring an explicit `compact.mode.set` request for Peek, so the Mini maximize button appeared dead when no sessions were active/waiting. Lua now tracks an ephemeral `idlePeekRequested` flag when the Mini enlarge button asks for Peek; idle still defaults back to Mini unless that explicit request is present. RED/GREEN confirmed with `phase_j_primary_peek.test.mjs`; full verification confirmed `node --test claude-tab-status/hammerspoon/*.test.mjs claude-tab-status/src/*.test.mjs` passing 183/183, `cargo build` passing, and `cargo test --no-run` passing.
- 2026-05-03: Run-scoped interrupt now also corrects Zellij tab rename state. Root cause: `row.interrupt` corrected only the Hammerspoon JSON/local interrupted-run view, while the live Zellij plugin could still keep the pane session as `Thinking`/`Tool`/`Waiting` and therefore keep the tab renamed as active. Hammerspoon now sends a run-scoped `ManualInterrupt` pipe for rows with `run_id`; Rust `HookPayload` accepts `run_id`, verifies it against the current pane session, marks that session `Done`, refreshes the affected tab name, and writes status without dismissing/blocking the pane. Legacy rows without `run_id` still use the existing pane denylist + `Dismiss` fallback. RED confirmed with new `src/manual_interrupt.test.mjs` and updated `phase_o_run_scoped_interrupt.test.mjs`; focused GREEN confirmed before full verification.
